...
class TestIngredientListMerge extends FlatSpec {
  ...
  "IngredientMerger" should "merge quantities of the same ingredient" in {
    val recipe1 = Recipe("chickpea curry",
      List(IngredientQuantity(Ingredient("cheese"), Quantity(20, MeasurementUnit.g), Freshness.fresh)))
    val recipe2 = Recipe("cheese sandwich",
      List(IngredientQuantity(Ingredient("cheese"), Quantity(5, MeasurementUnit.g), Freshness.fresh)))
    ...
    actual should contain only IngredientQuantityWithContributingRecipes(
      IngredientQuantity(Ingredient("cheese"), Quantity(25, MeasurementUnit.g), Freshness.fresh),
  ...
  it should "not merge different ingredients" in {
    val recipe1 = Recipe("chickpea-curry",
      List(IngredientQuantity(Ingredient("cheese"), Quantity(20, MeasurementUnit.g), Freshness.fresh)))
    val recipe2 = Recipe("cheese-sandwich",
      List(IngredientQuantity(Ingredient("bread"), Quantity(5, MeasurementUnit.g), Freshness.fresh)))
    ...
    actual should contain
      IngredientQuantityWithContributingRecipes(
        IngredientQuantity(Ingredient("cheese"), Quantity(20, MeasurementUnit.g), Freshness.fresh),
        List(recipe1, recipe2))

    actual should contain
      IngredientQuantityWithContributingRecipes(
        IngredientQuantity(Ingredient("bread"), Quantity(5, MeasurementUnit.g), Freshness.fresh),
        List(recipe1, recipe2))
